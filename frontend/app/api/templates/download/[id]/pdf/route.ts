...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const res = await fetch(`${CONTRACTS_SERVICE_URL}/api/templates/download/${id}/pdf`, {
  ...
  headers.set("Content-Disposition", res.headers.get("content-disposition") || `attachment; filename="template-${id}.pdf"`);
