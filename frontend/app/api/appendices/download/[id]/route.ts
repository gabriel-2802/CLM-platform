...
/**
 * Proxy for appendix PDF downloads.
 * Forwards the backend-issued JWT as Bearer so Spring's security filter passes.
 * Called from the browser as /api/appendices/download/{id}?type=unsigned|signed
 */
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
    ...
    const res = await fetch(`${CONTRACTS_SERVICE_URL}/api/appendices/download/${id}/${type}/pdf`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
    });

    if (!res.ok) {
        const errorText = await res.text();
        return new NextResponse(errorText || "Failed to download appendix", { status: res.status });
    }
