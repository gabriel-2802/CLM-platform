...
export default function ClientDetaliiForm({ initial, onSubmit, submitLabel = "Save" }: Props) {
  ...
  const [registruEvFiscala, setRegistruEvFiscala] = useState<string>(initial?.registruEvFiscala ?? "NU_E_CAZUL");
  const [accesDosarFiscal, setAccesDosarFiscal] = useState<string>(initial?.accesDosarFiscal ?? "NU_E_CAZUL");
  ...
  useEffect(() => {
    if (!initial) return;
    setRegistruEvFiscala(initial.registruEvFiscala ?? "NU_E_CAZUL");
    setAccesDosarFiscal(initial.accesDosarFiscal ?? "NU_E_CAZUL");
  ...
  return (
    <form action={handleAction} className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <Select value={registruEvFiscala} onValueChange={setRegistruEvFiscala}>
      ...
      <div>
        ...
        <Select value={accesDosarFiscal} onValueChange={setAccesDosarFiscal}>
          <SelectTrigger><SelectValue placeholder="Alege" /></SelectTrigger>
          <SelectContent>
            {enums.yesNoNa.map(v => <SelectItem key={v} value={v}>{v}</SelectItem>)}
          </SelectContent>
      ...
      <div className="md:col-span-2 mt-2">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <label className="inline-flex items-center gap-2 text-purple-800"><Checkbox checked={registruUC} onCheckedChange={(v) => setRegistruUC(!!v)} /><span>Registru UC</span></label>
          <label className="inline-flex items-center gap-2 text-purple-800"><Checkbox checked={ofSpalareBani} onCheckedChange={(v) => setOfSpalareBani(!!v)} /><span>Of spalare bani</span></label>
          <label className="inline-flex items-center gap-2 text-purple-800"><Checkbox checked={regulamentOrdineInterioara} onCheckedChange={(v) => setRegulamentOrdineInterioara(!!v)} /><span>Regulament OI</span></label>
          <label className="inline-flex items-center gap-2 text-purple-800"><Checkbox checked={manualPoliticiContabile} onCheckedChange={(v) => setManualPoliticiContabile(!!v)} /><span>Manual politici contabile</span></label>
          <label className="inline-flex items-center gap-2 text-purple-800"><Checkbox checked={adresaRevisal} onCheckedChange={(v) => setAdresaRevisal(!!v)} /><span>Adresa Revisal</span></label>
          <label className="inline-flex items-center gap-2 text-purple-800"><Checkbox checked={depunereDeclaratiiOnline} onCheckedChange={(v) => setDepunereDeclaratiiOnline(!!v)} /><span>Depunere declaratii online</span></label>
        </div>
      </div>
